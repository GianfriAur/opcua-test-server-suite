# OPC UA Test Server Suite — Documentation

A comprehensive OPC UA server suite designed for testing OPC UA client libraries. Built on `node-opcua` v2.x, deployed via Docker Compose as 8 independent server instances covering every major security and authentication scenario.
...
Client ──► opcua-no-security    (4840)  No security, anonymous only
       ...
       ──► opcua-legacy-security(4847)  Deprecated policies (Basic128, Basic256)
```
