...
|---|---|---|---|
| `SimpleEventType` | Every 5 seconds | 200 | `"Periodic event #N"` (N increments) |
| `SystemStatusEventType` | Every 15 seconds | 100 (600 for Maintenance) | `"System status: <state>"` |
...
2. Add an event monitored item with a `SimpleAttributeOperand` select clause:
   ...
   - `Severity` (UInt16)
3. Wait 5 seconds — you should receive a `SimpleEventType` event

#### Filtering events by type

Use a `ContentFilter` with `OfType` operator:
- Filter for `SimpleEventType` → receive events every 5s
- Filter for `SystemStatusEventType` → receive events every 15s
