...
All certificates are generated automatically by the `certs-generator` Docker container on first startup. They are stored in a shared Docker volume (`certs-volume`) mounted at `/app/certs/` inside each container.
...
```
URI:  urn:opcua:test-server:OPCUATestServer
DNS:  localhost
      opcua-no-security
      opcua-userpass
      opcua-certificate
...
Use `opcua-all-security` (port 4843) which has all 6 policies and all 3 modes enabled. This generates 11 endpoint combinations (6 policies × 2 secure modes + 1 None/None).

### Testing deprecated policies

Use `opcua-legacy-security` (port 4847) which only offers `Basic128Rsa15` and `Basic256`. Your client should ideally log a warning when using these.
